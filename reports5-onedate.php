...
// output the column headings
	fputcsv($output, array('Reference ID', 'Date', 'Item', 'Type', 'Class', 'Locale', 'Card #', 'Card Type', 'Before Tax', 'Tax', 'Total', 'Name', 'Age', 'Address', 'City', 'Province', 'Postal', 'Redeem Date', 'Transaction', 'Picked Up'));

// fetch the data
$a = mysqli_query($db,"SELECT * FROM oitems INNER JOIN items ON oitems.itemid = items.id INNER JOIN orders ON oitems.oid = orders.id where orders.status = 'complete' and oitems.redeemdate = '$date1' and items.cat <> '2' and oitems.void is NULL order by oitems.date desc");                                               
...
fputcsv($output, array($id, $date, $cat, $typ[$itemid], $idd[$itemid], $kiosk, $cnumb, $ctype, $amount, $tax, $amount2, $name, $age, $address, $city, $province, $postal, $redeem, 'purchase', $pri));	
	$i++;
}

...
$query = mysqli_query($db,"SELECT oitems.oid, oitems.printed, oitems.void, oitems.fullname, items.validafter, items.validbefore, items.description, oitems.redeemdate, oitems.id, oitems.purchprice, items.title, items.type, items.cat FROM oitems INNER JOIN items ON oitems.itemid = items.id INNER JOIN orders ON oitems.oid = orders.id where orders.status = 'complete' and oitems.redeemdate = '$date' and items.cat <> '2' and oitems.void is NULL order by oitems.date desc");                                               
...
$price = number_format($price, 2);
							...
							$new_array[] = array("id" => $oid, "title" => $title, "type" => $type, "price" => $price, "cat" => $cat, "redeemdate" => $redeemdate, "description" => $description, "vaft" => $vaft, "vbef" => $vbef, "fullname" => $fullname, "voi" => $void, "loca" => $loca, "print" => $print, "rid" => $rid);		
